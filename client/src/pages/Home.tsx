...
function PageView({ page, darkMode, notify, liveRun, history, onApprove, onReject }: { page: string; darkMode: boolean; notify: (message: string) => void; liveRun: PactlineRun | null; history: PactlineRun[]; onApprove: () => void; onReject: () => void }) {
  ...
  const isAudit = page === "Audit trail";
  const isIntent = page === "Intent plans";
  const heldAction = liveRun?.actions.find((action) => action.decision === "held");
  const queueCount = heldAction ? "01 pending" : "0 pending";
  const exportReport = async (kind: "audit" | "runs", format: "csv" | "pdf") => { try { const blob = await downloadReport(kind, format); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `pactline-${kind}.${format}`; anchor.click(); URL.revokeObjectURL(url); notify(`${kind === "audit" ? "Audit trail" : "Run history"} ${format.toUpperCase()} downloaded`); } catch (error) { notify(error instanceof Error ? error.message : "Report export failed"); } };
  ...
  const pageEvents = isIntent ? (liveRun?.plan.steps || []).map((step: any, index: number) => ({ time: `STEP ${String(index + 1).padStart(2, "0")}`, label: step.action || step.name || `Plan step ${index + 1}`, detail: step.target || "Allowed by captured intent", state: "allowed", icon: LockKeyhole })) : streamEvents;
  return <section className={`subpage ${darkMode ? "subpage-dark" : ""}`}>
    ...
    <div className="subpage-grid">
      <div className="subpage-panel wide"><div className="panel-header"><div><div className="micro-label">{isQueue ? "PENDING DECISION" : isAudit ? "CHRONOLOGICAL EVIDENCE" : isIntent ? "SIGNED AUTHORIZATION" : "SYSTEM STREAM"}</div><h2>{isQueue ? "Recipient outside plan" : isAudit ? "Audit events" : isIntent ? "Captured plan steps" : "Authorization events"}</h2></div><div className="panel-tools">{isAudit && <><button className="export-button" onClick={() => void exportReport("audit", "csv")}>CSV</button><button className="export-button" onClick={() => void exportReport("audit", "pdf")}>PDF</button></>}<span className={`page-badge ${isQueue ? "amber" : "blue"}`}>{isQueue ? queueCount : liveRun ? "Live" : "Waiting"}</span></div></div>
        {isQueue ? <><div className="queue-summary"><div className="risk-mark"><CircleAlert size={19} /></div><div><strong>{heldAction?.name || "No pending action"}</strong><span>{heldAction?.target || "Run the protected workflow to populate this queue"}</span></div><div className="queue-time">{heldAction ? "Held for review" : "No pending decision"}</div></div><div className="queue-copy">{heldAction ? "The agent proposed an action outside the captured intent. ArmorIQ has paused execution before the side effect." : "The approval queue is clear. Start a protected run to surface a decision."}</div>{heldAction ? <div className="decision-actions"><button className="reject-button" onClick={onReject}>Reject action</button><button className="approve-button" onClick={onApprove}><BadgeCheck size={15} /> Approve & resume</button></div> : <div className="decision-resolved"><ShieldCheck size={15} /><span>{liveRun?.status === "rejected" ? "Decision recorded. The action was cancelled before execution." : liveRun?.status === "approved" ? "Decision recorded. The approved action completed." : "No action is currently waiting for review."}</span></div>}</> : <div className="stream-list">{(pageEvents.length ? pageEvents : [{ time: "—", label: isIntent ? "No intent plan captured" : "No audit events loaded", detail: isIntent ? "Start a protected run to capture a signed authorization plan" : "Start a protected run to populate the backend audit trail", state: "allowed", icon: ShieldCheck }]).map((event, index) => { const Icon = event.icon; return <div className="stream-row" key={`${event.time}-${index}`}><div className={`stream-icon ${event.state}`}><Icon size={15} /></div><div><strong>{event.label}</strong><span>{event.detail}</span></div><time>{event.time}</time><span className={`page-badge ${event.state === "held" ? "amber" : "green"}`}>{event.state}</span></div>; })}</div>}
      </div>
      ...
      <div className="subpage-panel"><div className="panel-header"><div><div className="micro-label">{isIntent ? "CAPTURED AUTHORITY" : isAudit ? "EVIDENCE SCOPE" : "CURRENT CONTEXT"}</div><h2>{isIntent ? "Intent plan" : isAudit ? "Audit coverage" : "Intent plan"}</h2></div><LockKeyhole size={17} className="panel-icon" /></div><div className="plan-card"><div className="plan-card-top"><BadgeCheck size={15} /><span>{isAudit ? `${liveRun?.audit?.length || 0} evidence events` : liveRun?.plan.status || "Awaiting plan capture"}</span></div><strong>{isAudit ? (liveRun ? "Backend decision evidence" : "Audit evidence scope") : liveRun?.plan.goal || "Invoice handling plan"}</strong><p>{isAudit ? (liveRun ? "Every tool call, boundary decision, and human outcome is recorded in chronological order." : "Start a protected run to populate the backend audit trail.") : liveRun ? liveRun.plan.steps.map((step: any) => step.action || step.name || String(step)).join(" → ") : "Start a protected run to capture a task-specific authorization plan."}</p><div className="plan-meta"><span>{isAudit ? (liveRun?.runId || "Awaiting run") : liveRun?.plan.id || "Awaiting plan capture"}</span><span>{liveRun ? new Date(liveRun.createdAt).toLocaleTimeString() : "—"}</span></div></div><button className="outline-action" onClick={() => notify(liveRun ? `Intent ${liveRun.plan.id} loaded from the backend` : "Start a run to inspect captured authorization")} disabled={!liveRun}>Inspect authorization <ArrowUpRight size={14} /></button></div>
    </div>
  </section>;
}
  ...
  return (
    <div className={`app-shell ${darkMode ? "dark-mode" : ""} ${mobileMenuOpen ? "mobile-menu-open" : ""}`}>
      <aside className="sidebar">
        ...
        <button className="workspace-switch" onClick={() => window.location.assign("/settings")}><div className="workspace-avatar">PL</div><div><div className="workspace-name">Finance Ops</div><div className="workspace-meta">Protected workspace</div></div><ChevronDown size={14} /></button>
      ...
      <main className="main-canvas">
        ...
        <header className="topbar">
          ...
          <div className="top-actions"><button className="search-box" onClick={() => { setActiveNav("Live runs"); notify(`${runHistory.length} persisted run${runHistory.length === 1 ? "" : "s"} loaded`); }}><Search size={15} /><span>Search runs, invoices…</span><kbd>⌘ K</kbd></button><button className="icon-button theme-toggle" aria-label={darkMode ? "Switch to light mode" : "Switch to dark mode"} onClick={toggleDarkMode}>{darkMode ? <Sun size={17} /> : <Moon size={17} />}</button><button className="icon-button" aria-label="Notifications" onClick={() => { setActiveNav("Approval queue"); notify(heldAction ? "One approval requires attention" : "Approval queue is clear"); }}><Bell size={17} /><span className="notification-dot" /></button><button className="mobile-menu" aria-label={mobileMenuOpen ? "Close menu" : "Open menu"} onClick={() => setMobileMenuOpen((current) => !current)}><Menu size={18} /></button></div>
        </header>
